...
class ContrastiveLoRACLIPModel(torch.nn.Module):
    ...
    def forward(self, pixel_values, input_ids, attention_mask, labels=None, **kwargs):
        ...
        if labels is not None:
            ...
            contrastive_labels = torch.arange(batch_size, device=pixel_values.device, dtype=torch.long)
    ...
    def get_image_features(self, pixel_values):
        return F.normalize(self.clip_model.get_image_features(pixel_values=pixel_values), dim=-1)

    def get_text_features(self, input_ids, attention_mask):
        return F.normalize(self.clip_model.get_text_features(
...
def train_few_shot(args):
    ...
    run_parent_dir = os.path.join(args.output_dir, f"run{run_id}_lora_r{args.lora_r}_alpha{args.lora_alpha}_vision_text")
    ...
    for shot in shot_values:
        ...
        model = ContrastiveLoRACLIPModel(args.clip_model_name, lora_config).to(device)
        model.train()
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if args.save_each_epoch:
                ...
                model.clip_model.save_pretrained(epoch_save_dir)
                processor.save_pretrained(epoch_save_dir)

            if is_best:
                best_save_dir = os.path.join(shot_output_dir, "best_model")
                os.makedirs(best_save_dir, exist_ok=True)
                model.clip_model.save_pretrained(best_save_dir)
                ...
                torch.save({
                    ...
                    "epoch": epoch
                }, os.path.join(best_save_dir, "training_config.pth"))

                print(f"  -> Saved best LoRA model for shot={shot} to {best_save_dir} (val_auc={monitor_metric:.4f})")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Few-shot CLIP LoRA training with Similarity Gap Analysis")
    ...
    parser.add_argument(
        "--target_modules",
        type=str,
        default="vision_model.encoder.layers.11.self_attn.q_proj,vision_model.encoder.layers.11.self_attn.v_proj,text_model.encoder.layers.11.self_attn.q_proj,text_model.encoder.layers.11.self_attn.v_proj",
        help="Comma-separated target modules for LoRA injection (supports both vision and text modules)"
    )
